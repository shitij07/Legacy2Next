...
def upgrade() -> None:
    ...
    sa.Enum('MARKDOWN', 'JSON', name='reportformat').create(op.get_bind(), checkfirst=True)
    op.add_column('reports', sa.Column('format', sa.Enum('MARKDOWN', 'JSON', name='reportformat'), nullable=False, server_default='MARKDOWN'))
    sa.Enum('GENERATING', 'READY', 'FAILED', name='reportstatus').create(op.get_bind(), checkfirst=True)
    ...
    op.create_foreign_key('fk_reports_analysis_id', 'reports', 'analyses', ['analysis_id'], ['id'])
